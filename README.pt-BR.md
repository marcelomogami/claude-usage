...
| `waybar` | JSON com `text`, `tooltip` e `class` para módulo customizado da Waybar |
| `all` (padrão) | `<uso>::<status>` — formato consumido pelo plasmoid |

## Instalação

```bash
# 1. clonar o repositório
git clone https://github.com/marcelomogami/claude_usage.git ~/projects/personal/claude_usage
...
> **Nota:** o caminho do script está hardcoded no `plasmoid/contents/ui/main.qml`
> (`scriptBase`). Se você clonar o repositório em outro diretório, ajuste essa linha.

### Waybar

Adicione um módulo customizado ao `~/.config/waybar/config.jsonc`:
...
- KDE Plasma 6 (para o plasmoid) ou Waybar (para o módulo customizado)
- `jq` e `curl`
- Claude Code instalado e autenticado (`~/.claude/.credentials.json`)

## Changelog

Ver [CHANGELOG.pt-BR.md](CHANGELOG.pt-BR.md).

## Licença

[MIT](LICENSE) © Marcelo Mogami

Projeto pessoal, sem qualquer vínculo com a Anthropic. "Claude" é marca da Anthropic.
